...
export default function PlayerPage() {
  ...
  async function handleSelectEpisode(epId: number) {
    try {
      setSelectedEpisode(epId);
      setVideoUrl(null); // reset temporar
      ...
      if (error) throw error;
      if (!data) {
        setError("Nu există sursă video pentru acest episod.");
        return;
      }

      setVideoUrl(data.source_url); // salvăm doar link-ul
      setError(null);
    } catch (err) {
      console.error("Eroare la handleSelectEpisode:", err);
      setError("Eroare la încărcarea sursei video.");
    }
  }
      ...
      <div className="p-6 flex flex-col">
        ...
        <div className="w-full flex flex-row gap-2 mt-2">
          ...
          <button className="w-full px-5 py-2 text-sm border border-[#05c149] text-[#05c149] rounded-full justify-center flex items-center gap-2 hover:cursor-pointer">
            <Plus color="#05c149" /> Add to my List{" "}
          </button>
        </div>
          ...
          {episodes.map((ep) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 w-full p-2">
                ...
                <p className="text-gray-200 text-sm z-40">
                  Episodul {ep.episode_number}
                </p>
              </div>
            </div>
          ))}
